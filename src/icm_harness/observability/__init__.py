"""Public interface (barrel) for the ``observability`` module.

Other modules must import these symbols from ``icm_harness.observability`` — never from
...
def __getattr__(name: str) -> Any:
    ...
    return getattr(importlib.import_module(f"icm_harness.observability.{module}"), name)
