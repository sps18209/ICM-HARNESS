"""Public interface (barrel) for the ``modes`` module.

Other modules must import these symbols from ``icm_harness.modes`` — never from
...
def __getattr__(name: str) -> Any:
    ...
    return getattr(importlib.import_module(f"icm_harness.modes.{module}"), name)
