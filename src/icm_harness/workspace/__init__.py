"""Public interface (barrel) for the ``workspace`` module.

Other modules must import these symbols from ``icm_harness.workspace`` — never from
...
def __getattr__(name: str) -> Any:
    ...
    return getattr(importlib.import_module(f"icm_harness.workspace.{module}"), name)
