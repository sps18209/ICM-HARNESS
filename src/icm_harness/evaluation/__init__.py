"""Public interface (barrel) for the ``evaluation`` module.

Other modules must import these symbols from ``icm_harness.evaluation`` — never from
...
def __getattr__(name: str) -> Any:
    ...
    return getattr(importlib.import_module(f"icm_harness.evaluation.{module}"), name)
